...
def infer_arch_from_name(name: str) -> str | None:
    ...
    if "resnet18" in n or "resnet-18" in n or "rn18" in n:
        return "resnet18"
    if "deit" in n or "vit_tiny" in n or "vit-tiny" in n or "vittiny" in n or "vit" in n and "tiny" in n:
        return "deit-tiny"
...
def main():
    ...
    ap.add_argument("--models_dir", default="models", help="Folder containing model files (.onnx or .pt)")
    ap.add_argument("--cam", type=int, default=0, help="Camera index (default 0)")
    ap.add_argument("--size", type=int, default=224, help="Model input size (default 224)")
    ap.add_argument("--ema", type=float, default=0.25, help="EMA smoothing alpha (default 0.25)")
    ...
    if not app_path.exists():
        sys.exit(f"[run.py] Could not find {APP} next to run.py. Make sure it’s in the same folder.")
    ...
    all_files = sorted([p for p in models_dir.iterdir() if p.suffix.lower() in (".onnx", ".pt")])
    if args.only:
    ...
    if not model_files:
        sys.exit(f"[run.py] No model files found in {models_dir.resolve()} "
    ...
    for p in model_files:
        if p.suffix.lower() == ".onnx":
            ...
            if arch is None:
                # No supported arch inference → skip with a warning
    ...
    if skipped:
        print("[run.py] Skipped .pt files (unknown arch, export Mini-X to ONNX or rename to include resnet18/deit):")
        for s in skipped:
            print("   -", s)

    # Quick dependency checks (optional but friendly)
    if onnx_needed and not _module_available("onnxruntime"):
        print("[run.py] WARNING: onnxruntime not installed; ONNX models will fail. Install: pip install onnxruntime")
    if torch_needed:
        if not _module_available("torch"):
            print("[run.py] WARNING: torch not installed; PyTorch models will fail. Install: pip install torch torchvision")
        if not _module_available("timm"):
            print("[run.py] WARNING: timm not installed; required for PyTorch model creation. Install: pip install timm")
